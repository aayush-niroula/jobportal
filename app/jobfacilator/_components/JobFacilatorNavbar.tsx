      ...
      <div className="flex items-center justify-between lg:justify-center max-w-7xl mx-auto">
        ...
        <Link href={"/jobfacilator"}>
          <div className="flex items-center gap-3 shrink-0">
            <img
              src="/Talent.png"
              alt="logo"
              className="h-10 w-10 sm:h-12 sm:w-12 lg:h-15 lg:w-15 object-contain"
            />
        ...
        <div className="hidden lg:flex flex-1 justify-center">
          <ul className="flex items-center gap-3 md:gap-5 lg:gap-10 text-sm md:text-base lg:text-lg">
            <Link href={`/jobfacilator/myjobs/${user?.id}`}>
              <li className="cursor-pointer nav-underline">My Jobs</li>
            </Link>
            <Link href={"/jobfacilator/candidates"}>
              <li className="cursor-pointer nav-underline">Candidates</li>
            </Link>
            <Link href={"/jobfacilator/events"}>
              <li className="cursor-pointer nav-underline">Events</li>
            </Link>
            <Link href={`/jobfacilator/postajob/${id}`}>
              <li className="cursor-pointer nav-underline">Post a New Job</li>
            </Link>
          </ul>
        </div>
        ...
        <div className="hidden lg:flex lg:gap-4 items-center">
          <div className="flex items-center gap-3 sm:gap-4 ml-2 sm:ml-4">
            <AvatarDropDown />
            <h1 className="text-xl font-medium whitespace-nowrap">{user?.name}</h1>
          </div>
          <Button onClick={handleLogout} className="text-lg px-6">
            Logout
          </Button>
        </div>

        {/* Mobile Menu Toggle */}
        <button className="lg:hidden flex items-center" onClick={() => setOpen(!open)}>
      ...
      {open && (
        <div className="lg:hidden mt-3 w-full bg-[#F8F9FA] shadow-md rounded-xl p-4 flex flex-col items-center gap-4 text-base z-50">
          ...
          <div className="flex flex-col items-center gap-2 mb-4">
            <AvatarDropDown />
            {user?.name && <span className="font-medium text-base">{user.name}</span>}
          </div>
          ...
          {/* Logout */}
          <Button className="w-full text-lg py-3" onClick={handleLogout}>
            Logout
          </Button>
        </div>
      )}
